...
CREATE POLICY "Allow public read access to gita_chapters" ON gita_chapters FOR SELECT USING (true);
CREATE POLICY "Allow public read access to gita_verses" ON gita_verses FOR SELECT USING (true);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'gita_bookmarks' AND policyname = 'Users can read their own bookmarks'
  ) THEN
    CREATE POLICY "Users can read their own bookmarks" ON gita_bookmarks FOR SELECT USING (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'gita_bookmarks' AND policyname = 'Users can insert their own bookmarks'
  ) THEN
    CREATE POLICY "Users can insert their own bookmarks" ON gita_bookmarks FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public' AND tablename = 'gita_bookmarks' AND policyname = 'Users can delete their own bookmarks'
  ) THEN
    CREATE POLICY "Users can delete their own bookmarks" ON gita_bookmarks FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
